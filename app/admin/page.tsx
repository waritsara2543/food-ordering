...
import { Download, Edit, Plus, Trash2, Save, X, Check, Clock } from "lucide-react"
import Link from "next/link"
import { motion, AnimatePresence } from "framer-motion"
import { useRouter } from "next/navigation"
import { menuItemsApi, ordersApi, type MenuItem, type OrderWithItems } from "@/lib/supabase"
...
export default function AdminPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const [ordersData, menuData] = await Promise.all([ordersApi.getAll(), menuItemsApi.getAll()])
  ...
  const exportTodayOrders = () => {
    ...
    const csvContent = [
      ...
      ...todayOrders.map((order) => [
        ...
        order.order_items.map((item) => `${item.menu_item_name} x${item.quantity}`).join(", "),
        order.total.toString(),
        order.payment_status === "approved" ? "อนุมัติ" : order.payment_status === "rejected" ? "ปฏิเสธ" : "รอตรวจสอบ",
      ]),
    ]
      .map((row) => row.join(","))
      .join("\n")
    ...
    link.setAttribute("download", `orders_${new Date().toLocaleDateString("th-TH")}.csv`)
  ...
  const handleUpdatePaymentStatus = async (orderId: string, status: "approved" | "rejected") => {
    try {
            ...
            <TabsContent value="orders" className="space-y-6">
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}>
                <Card className="bg-white/80 backdrop-blur-sm shadow-xl border-0">
                  <CardHeader>
                    <CardTitle className="flex items-center text-gray-800">📅 รายการสั่งซื้อวันนี้</CardTitle>
                  </CardHeader>
                  <CardContent>
                    {getTodayOrders().length === 0 ? (
                      ...
                      >
                        ...
                        <p className="text-gray-500 text-lg">ไม่มีรายการสั่งซื้อวันนี้</p>
                      </motion.div>
                    ) : (
                        ...
                        <AnimatePresence>
                          {getTodayOrders().map((order, index) => (
                            <motion.div
                              ...
                              whileHover={{ scale: 1.02, backgroundColor: "#fefefe" }}
                            ...
                            >
                              ...
                              {order.payment_slip && (
                                <div className="mb-3">
                                  <p className="text-sm font-medium text-gray-700 mb-2">สลิปการโอนเงิน:</p>
                                  <img
                                    src={order.payment_slip || "/placeholder.svg"}
                                    alt="สลิปการโอนเงิน"
              ...
              <AnimatePresence>
                {isAddingNew && (
                  ...
                  >
                    <Card className="bg-white/80 backdrop-blur-sm shadow-xl border-0">
                      <CardHeader>
                        <CardTitle className="flex items-center text-gray-800">➕ เพิ่มรายการใหม่</CardTitle>
                      </CardHeader>
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          >
                            <Label htmlFor="newCategory" className="text-gray-700">
                              หมวดหมู่
                            </Label>
                            <Select
                              value={newItem.category}
                              onValueChange={(value) =>
                                setNewItem((prev) => ({ ...prev, category: value as "drinks" | "food" }))
                              }
                            >
                        ...
                        >
                          <Label htmlFor="newDescription" className="text-gray-700">
                            คำอธิบาย
                          </Label>
                          <Input
                            id="newDescription"
                        ...
                        <CardContent className="p-4">
                          {editingItem?.id === item.id ? (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                              ...
                              <div>
                                ...
                                <Select
                                  ...
                                  onValueChange={(value) =>
                                    setEditingItem((prev) =>
                                      prev ? { ...prev, category: value as "drinks" | "food" } : null,
                                    )
                                  }
                                >
                              ...
                              <div>
                                <Label className="text-gray-700">คำอธิบาย</Label>
                                <Input
                            ...
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                              ...
                              <motion.p className="text-xl font-bold text-green-600 mb-4" whileHover={{ scale: 1.05 }}>
                                ฿{item.price}
                              </motion.p>
                              <Badge
                                variant="outline"
                              ...
                              >
                                {item.category === "drinks" ? "🥤 เครื่องดื่ม" : "🥪 อาหาร"}
                              </Badge>
                              <div className="flex space-x-2">
                                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                                  <Button
